...
class Database:
    ...
    # Interesting messages methods

    async def add_interesting_message(
        self,
        message_id: str,
        channel_id: str,
        guild_id: str,
        ...
        """Add an interesting message to the pool. Returns True if inserted."""
        try:
            await self.execute(
                """
                INSERT OR IGNORE INTO interesting_messages
                (message_id, channel_id, guild_id, author_id, content, timestamp_ms, interest_score)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    message_id,
            ...
            logger.error(f"Failed to add interesting message: {e}")
            return False

    async def update_interest_score(
        self, message_id: str, additional_flags: int
    ) -> None:
        """Update interest score by adding flags."""
        await self.execute(
            """
            UPDATE interesting_messages
            SET interest_score = interest_score | ?
            WHERE message_id = ?
            """,
            (additional_flags, message_id),
        )

    async def get_random_message(
        self, guild_id: str, min_age_ms: int
    ) -> Optional[aiosqlite.Row]:
        """Get a random interesting message older than min_age_ms."""
        import time

        max_timestamp = int(time.time() * 1000) - min_age_ms

        return await self.fetch_one(
            """
            SELECT * FROM interesting_messages
            WHERE guild_id = ? AND timestamp_ms < ?
            ORDER BY RANDOM()
            LIMIT 1
            """,
            (guild_id, max_timestamp),
        )

    async def count_interesting_messages(
        self, guild_id: str, min_age_ms: int
    ) -> int:
        """Count interesting messages for a guild that are old enough."""
        import time
        ...
        result = await self.fetch_value(
            """
            SELECT COUNT(*) FROM interesting_messages
            WHERE guild_id = ? AND timestamp_ms < ?
            """,
            (guild_id, max_timestamp),
        )
